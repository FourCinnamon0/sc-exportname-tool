...
if([null, 1, 3].includes(version)) {
    try{
        if (Buffer.compare(data.slice(0,4), Buffer.from("SCLZ", 'utf8')) == 0) {
            ...
            console.log('[*] Compressão Zstandard detectada !')
			const ZstdCodec = require('zstd-codec').ZstdCodec
            decompressed = (new ZstdCodec.Simple()).decompress(data)

        }else{
			const lzma = require("lzma");
            console.log('[*] Compressão LZMA detectada !')
        ...
        console.error(e)
        console.log(`Não é possível descompactar o arquivo!`)
        return;
    }
} else {
    decompressed = data
...
if(saveToFile) {
	clog = function (...args) {
		...
		console.log("Saída salva em export_names.txt")
	}
}else{
	clog = function (...args) {
		console.log(...args)
